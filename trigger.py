...
def trigger(isSend, setTimeDict, callback, *params):
    ...
    setTimeStr = str(nowArray.tm_year) + "-" + str(nowArray.tm_mon) + "-" + str(nowArray.tm_mday) + " " + \
        str(setTimeDict["h"]) + ":" + \
        str(setTimeDict["m"]) + ":" + str(setTimeDict["s"])
    ...
    if now < setTime:
        ...
        nextDayTime = time.mktime(time.strptime(
            nextDayStr, "%Y-%m-%d %H:%M:%S"))
    ...
    threading.Timer(remaining, trigger,
                    (isSend, setTimeDict, callback, *params)).start()
